...
目前处于 **单 hart、同步 RAM 的 RV32IMA 五级流水原型** 阶段，能够运行内置指令数组，观察逐拍执行过程并运行定向测试，并已具备 CSR 寄存器文件与 M/S/U 特权级骨架。系统级运行和 Linux 启动仍是后续目标。
...
- **CSR 与特权级**：提供 4096 项 CSR 文件与 M/S/U 特权级状态（复位进入 M 模式），实现 `csrrw/csrrs/csrrc/csrrwi/csrrsi/csrrci`。按 CSR 地址位编码检查读写权限与最低可访问特权级：写只读 CSR 或特权级不足均按非法指令处理；`mcounteren` 门控 U 模式的计数器访问。读写副作用与 store 一致延迟到 WB，`rd` 写回动作前的旧值。`misa` 为 WARL，`time` 暂与 `cycle` 同源（尚未接入 CLINT）。
...
- **异常报告**：支持非法指令、访问越界、地址不对齐、ECALL 和 EBREAK。异常指令到达 WB 时记录原因、PC 与故障值并停止模拟，不计入退休数；非法 CSR 访问走同一路径。尚无陷阱向量、`mret/sret`、中断与委托。
- **观测与测试**：记录周期数、退休数、数据相关停顿、EX 乘除等待拍数、MEM 阻塞拍数和重定向次数；提供 RV32IM/A/CSR 定向测试（含逐拍状态断言与 Python 端到端测试），以及阶段日志转换、比较和 HTML 展示工具。
...
目前尚未支持 ELF 装载、FENCE.I、MMU 及完整系统设备、缓存或可变访存延迟，也尚未接入 HDL 验证；CSR 仅有寄存器文件与访问权限骨架，尚无陷阱处理与 `mret/sret`。原子操作目前只面向单 hart 独占 RAM，不支持多个核心或 DMA 并发修改同一 RAM。原 `tools/diff.py` 仍比较阶段事件；新增 `tools/spike_compare.py` 以 Spike 为参考模型差分正常提交的寄存器与内存写结果。
...
4. **探索系统级运行**：在已有 CSR 与特权级骨架上补齐陷阱处理（`mepc/mcause/mtvec`、`mret/sret`、中断与委托），再实现 Sv32 地址转换及必要外设，最终争取由周期模型启动 Linux 并进入 shell。
