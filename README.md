...
- **内存与提交**：指令数组和数据 RAM 分离，数据按小端访问；寄存器写回与普通 store 在 WB 生效。原子操作到达 MEM 时较老指令已提交，原子写入在 MEM 最后一拍生效，WB 只写寄存器和退休；x0 保持为零。
...
目前尚未支持 ELF 装载、CSR、FENCE.I、MMU、MMIO、缓存或可变访存延迟，也尚未接入 Spike 或 HDL 验证。原子操作目前只面向单 hart 独占 RAM，不支持多个核心或 DMA 并发修改同一 RAM。Python 工具比较的是阶段事件，不是寄存器与内存的架构状态差分。
