...
void in_core_decode(INCore *core)
{
    ...
    switch (op) {
    case OPCODE_LUI: case OPCODE_AUIPC: /* LUI, AUIPC */
        l->writes_rd = 1;
    ...
    case OPCODE_OP_IMM: /* immediate ALU */
        ...
        if (f3 == 1) legal = f7 == 0;
        if (f3 == 5) legal = f7 == 0 || f7 == 0x20;
        break;
    case OPCODE_OP: /* register ALU */
        use1 = use2 = l->writes_rd = 1;
        /* 寄存器运算中，仅 SUB/SRA 允许 funct7=0100000。 */
        legal = f7 == 0 || (f7 == 0x20 && (f3 == 0 || f3 == 5));
        break;
    case OPCODE_MISC_MEM: /* FENCE：单核同步 RAM 不需要额外动作。 */
        legal = f3 == 0;
        break;
    case OPCODE_SYSTEM:
        if (i == 0x00000073u) l->exception = 12; /* M-mode ECALL, cause 11 */
        else if (i == 0x00100073u) l->exception = 4; /* breakpoint */
        else legal = 0;
